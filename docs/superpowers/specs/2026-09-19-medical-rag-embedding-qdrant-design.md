...
- `EmbedClient` treats tunnel errors (`502`, `503`, `504`, `524`, `530`; Cloudflare error `1033` arrives as HTTP `530`) and connection errors or timeouts as transient: up to 3 attempts with doubling backoff. Other non-200 responses fail immediately, and so do non-transient `requests` errors (for example a URL without a scheme) and malformed or wrongly shaped bodies: every failure surfaces as `EmbedError`, never as a raw `requests` exception.
...
- `terms(segmented: str) -> list[str]` and `term_index(term: str) -> int` — the tokenizer and the hash behind the BM25 encoder (see the BM25 section); public because `validate_ingest` and a future retrieval stage use them.
...
- `ensure_collection(client, recreate: bool = False) -> None` — creates the collection if missing; if it exists it must have a `dense` vector of size 768 and a `sparse` vector with `Modifier.IDF`, otherwise it raises and does not recreate (a collection without the IDF modifier would silently produce a wrong sparse index); `recreate=True` drops and rebuilds.
...
- `main()` CLI: `--data-dir` (default `data`), `--qdrant-path` (default `qdrant_data`), `--embed-url` (default `$EMBED_URL`), `--recreate`. Exit codes: `0` success; `1` at least one article failed; `2` configuration or startup failure (missing `EMBED_URL`, dead tunnel, locked or unusable Qdrant path, existing collection with a different dense size, missing data directory, empty corpus, tokenizer load failure), always with a printed message and never a traceback.
...
3. **Pass 2 (per article)**: embed the article's segmented chunks in batches, build sparse vectors, build points, then `replace_article`. Embedding finishes **before** any deletion, so a failed embedding leaves the article's old points untouched. A failed embedding is recorded and processing continues with the next article. A failure inside Qdrant itself (`replace_article` raising) is different: `replace_article` deletes before it upserts, and a failed upsert can leave a local-mode collection half-written (observed: a wrong-size vector left the collection so that the next valid article failed with `IndexError`). Five embedding failures in a row (`MAX_CONSECUTIVE_EMBED_FAILURES`) also stop the run: a hung Cloudflare tunnel costs about 183 s per article (3 attempts x 60 s timeout plus backoff), so continuing would burn hours on articles that cannot succeed, and the message says to re-run after restarting the notebook. So a store error is recorded and **stops the run** with a message to re-run with `--recreate`; the affected article may be missing until then, and the summary's expected-versus-collection point counts show the gap.
...
- `test_ingest.py`: whole flow on a temporary data directory, a deterministic 768-dimension fake embedder and `:memory:` Qdrant — point counts, payload fields, one article's embedding failing leaves the others ingested and its old points intact, a store failure recorded and stopping the run, and the exit-2 cases of `main` (missing `EMBED_URL`, dead tunnel, URL without a scheme, missing data directory, empty corpus, locked Qdrant path). The real localhost HTTP server helper lives in `backend/tests/fake_server.py`, shared with `test_encoders.py`.
...
- Exit codes: `0` all invariants pass; `1` an invariant problem (or, once floors exist, a hit-rate below its floor); `2` the environment is broken (missing `EMBED_URL`, dead tunnel, locked or missing Qdrant path or collection, empty data directory), with a printed `validate aborted: ...` message. The tunnel health check runs first, so a dead tunnel fails in seconds instead of after the two minutes of chunking.
